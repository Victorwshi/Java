        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                for (int direction = 1; direction < 9; direction++) {
                    ...
                    int xValue = i + dx(direction);
                    int yValue = j + dy(direction);
                    while ((xValue < width) && (yValue < height)
                                && (xValue >= 0) && (yValue >= 0)) {
                        M[i][j][direction].add(Place.pl(xValue, yValue));
                        M[i][j][0].add(Place.pl(xValue, yValue));
                        xValue += dx(direction);
                        yValue += dy(direction);
                    }
                }
