    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                ArrayList<Sq> find = new ArrayList<Sq>();
                for (Sq square : model) {
                    if (sq.connectable(square)) {
                        find.add(square);
                        nFound += 1;
                    }
                }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                ArrayList<Sq> find2 = new ArrayList<Sq>();
                for (Sq square : model) {
                    if (square.connectable(sq)) {
                        find2.add(square);
                        nFound += 1;
                    }
                }
